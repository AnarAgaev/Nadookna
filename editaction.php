		...
		<div class="content">
			...
			<div class="title_vhod">
				<div>
					...
					<h1 class="tit_date">Редактирование Акции #<?php echo $_GET['id_action'] ?></h1>
				</div>
			</div>
			
			<?php
				// Получим данные Акции
				
				$id_action = $_GET['id_action'];
				$res = mysql_query("SELECT `id_action`, `title`, `text`, `data_start`, `data_finish` FROM `action` WHERE `id_action` = '$id_action'",$db);
			...
			<div class="dateContent">
				...
				<div class="boxEditData">
					...
					<div class="datepicker_conteiner"><input type="text" name="dsa" id="data_start_action" class="datepicker" style="cursor: pointer;" form="form_editAction" value="<?php echo date_format(date_create($row['data_start']), 'd.m.Y'); ?>" readonly></div>
					
					<div class="titelPointEditData">Дата окончания Акции</div>
					<div class="datepicker_conteiner"><input type="text" name="dfa" id="data_finish_action" class="datepicker" style="cursor: pointer;" form="form_editAction" value="<?php echo date_format(date_create($row['data_finish']), 'd.m.Y'); ?>" readonly></div>
					
					
					<div class="saveEditData roundBorder" onclick="f_edit_action()">Сохранить изменения</div>
				</div>		
			</div>
